...
function action_CHALLENGE_FAIL_LevelChange(context,evt)
    ...
    for i = 1,#potList do
        local targetPot = potList[i]
        -- 只要锅子不是处于202或203
        local targetPotState = ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, targetPot)
...
function action_PotOverState(context, evt)
    ...
    if 201 ~= ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, targetConfigID) then
...
function StoveMoveByButtonState(context)
    local leftState = ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, defs.ButtonLeftConfig)
    local rightState = ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, defs.ButtonRightConfig)
    local curPos = ScriptLib.GetGroupVariableValue(context, "curPos")
