...
function LF_Check_Water_To_Motion(context, evt)
	local up_state = ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, defs.gadget_up)
	local down_state = ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, defs.gadget_down)
	local water = ScriptLib.GetGroupVariableValue(context, "Water_Level")
