...
function action_EVENT_ENTER_REGION_61004(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标{x=-2953.98, y = 60.767, z=4612.459}，持续时间为3秒，并且为强制注目形式，不广播其他玩家
		local pos = {x=-2953.980, y=60.767, z=4612.459}
...
function action_EVENT_TIME_AXIS_PASS_61010(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标{x=-2982.271, y = 21.100, z=4436.112}，持续时间为3秒，并且为强制注目形式，不广播其他玩家
		local pos = {x=-2982.271, y=21.100, z=4436.112}
	  local pos_follow = {x=0, y=0, z=0}
	    if 0 ~= ScriptLib.BeginCameraSceneLook(context, { look_pos = pos, is_allow_input = true, duration = 3, is_force = true, is_broadcast = false, is_recover_keep_current = true, delay = 0,
	                                                      is_set_follow_pos = false, follow_pos = pos_follow, is_force_walk = true, is_change_play_mode = false,
	                                                      is_set_screen_XY = false, screen_x = 0, screen_y = 0 }) then
