...
function condition_EVENT_GADGET_CREATE_134009(context, evt)
	if 134001 ~= evt.param1 or GadgetState.Default ~= ScriptLib.GetGadgetStateByConfigId(context, base_info.group_id, evt.param1) then
		return false
	end
...
function action_EVENT_GADGET_STATE_CHANGE_134012(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标（0，0，0），持续时间为1秒，并且为强制注目形式，不广播其他玩家
		local pos = {x=0.000, y=0.000, z=0.000}
