...
function action_EVENT_ANY_MONSTER_DIE_46013(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标{x=-2065.061, y = 192.474, z=5038.861}，持续时间为2秒，并且为强制注目形式，不广播其他玩家
		local pos = {x=-2065.061, y=192.474, z=5038.861}
...
function action_EVENT_ANY_MONSTER_DIE_46027(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标{x=-2059.898, y = 202.423, z=5066.739}，持续时间为2秒，并且为强制注目形式，不广播其他玩家
		local pos = {x=-2059.898, y=202.423, z=5066.739}
	  local pos_follow = {x=0, y=0, z=0}
	    if 0 ~= ScriptLib.BeginCameraSceneLook(context, { look_pos = pos, is_allow_input = true, duration = 2, is_force = true, is_broadcast = false, is_recover_keep_current = true, delay = 0,
	                                                      is_set_follow_pos = false, follow_pos = pos_follow, is_force_walk = false, is_change_play_mode = false,
	                                                      is_set_screen_XY = false, screen_x = 0, screen_y = 0 }) then
